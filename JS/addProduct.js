...
const selectBanType = document.getElementById('selectBanType');
...
// Add all years
const years = [];
for (let year = 2025; year >= 1905; year--) {
  years.push(year);
}
for (const year of years) {
  yearCode += `<option>${year}</option>`;
}
selectYear.innerHTML += yearCode;

// Add all bans 
const bans = [];
products.forEach(product => {
  if (!bans.includes(product.banType)) {
    bans.push(product.banType);
  }
});
for(const ban of bans) {
   banCode += `<option>${ban}</option>`
  }
selectBanType.innerHTML += banCode;

// Add all marka
const markalar = [];
products.forEach(product => {
  if(!markalar.includes(product.brand)){
    markalar.push(product.brand)
  }
})

markalar.sort();
for(const marka of markalar){
  markaCode += `<option value="${marka}">${marka}</option>`
}
selectMarka.innerHTML += markaCode;

selectMarka.addEventListener('change', function() {
  filterModel(this.value);
});

// Filter brand to model
function filterModel(marka) {
  const models = [];
  products.forEach(product => {
    if (product.brand === marka && !models.includes(product.model)) {
      models.push(product.model);
    }
  });
  modelCode = '<option value="noOption">Seçin</option>';
  for (const model of models) {
    modelCode += `<option>${model}</option>`;
  }
  selectModel.innerHTML = modelCode;
}
...
const azerbaijanCities = [
  ...
  "Astara", "Balakən", "Beyləqan", "Biləsuvar",
  "Cəbrayıl", "Cəlilabad", "Daşkəsən", "Füzuli", "Gədəbəy",
  "Gəncə", "Goranboy", "Göyçay", "Göygöl", "Hacıqabul",
  "İmişli", "İsmayıllı", "Kəlbəcər", "Kürdəmir", "Laçın",
  "Lənkəran", "Lerik", "Masallı", "Mingəçevir", "Naftalan",
  "Naxçıvan", "Neftçala", "Oğuz", "Qəbələ", "Qax",
  "Qazax", "Quba", "Qubadlı", "Qusar", "Saatlı",
  "Sabirabad", "Şabran", "Salyan", "Şamaxı", "Şəki",
  "Şəmkir", "Samux", "Şirvan", "Siyəzən", "Sumqayıt",
  "Şuşa", "Tərtər", "Tovuz", "Ucar", "Xaçmaz",
  "Xankəndi", "Xızı", "Xocalı", "Xocavənd", "Yardımlı",
...
azerbaijanCities.sort()
for (const city of azerbaijanCities) {
  selectCode += `<option value="${city}">${city}</option>`
}
contactSelect.innerHTML += selectCode;
